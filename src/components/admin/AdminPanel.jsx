          ...
          <div className="ml-5 mt-2 space-y-1 animate-in slide-in-from-top-2 duration-300">
            {item.children.map((child) => (
              <Link
                key={child.title}
                to={child.url}
                className={`group flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 hover:scale-[1.01] ${
                  isActive(child.url)
...
// Main Sidebar Component
export default function AdminNav({ isOpen, onToggle, className = "" }) {
